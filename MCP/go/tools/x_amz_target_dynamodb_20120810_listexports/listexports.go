...
func ListexportsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ListExportsInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.ListExports%s", cfg.BaseURL, queryString)
		...
		var result models.ListExportsOutput
...
func CreateListexportsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_ListExports",
		mcp.WithDescription("Lists completed exports within the past 90 days."),
		...
		mcp.WithString("MaxResults", mcp.Description("Input parameter: Maximum number of results to return per page.")),
		mcp.WithString("NextToken", mcp.Description("Input parameter: An optional string that, if supplied, must be copied from the output of a previous call to <code>ListExports</code>. When provided in this manner, the API fetches the next page of results.")),
		mcp.WithString("TableArn", mcp.Description("Input parameter: The Amazon Resource Name (ARN) associated with the exported table.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListexportsHandler(cfg),
	}
}
