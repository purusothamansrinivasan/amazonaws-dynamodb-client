...
func DescribeimportHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DescribeImportInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.DescribeImport", cfg.BaseURL)
		...
		var result models.DescribeImportOutput
...
func CreateDescribeimportTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_DescribeImport",
		mcp.WithDescription(" Represents the properties of the import. "),
		...
		mcp.WithString("ImportArn", mcp.Required(), mcp.Description("Input parameter:  The Amazon Resource Name (ARN) associated with the table you're importing to. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DescribeimportHandler(cfg),
	}
}
