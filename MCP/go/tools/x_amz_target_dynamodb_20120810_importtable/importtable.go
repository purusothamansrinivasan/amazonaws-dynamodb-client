...
func ImporttableHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ImportTableInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.ImportTable", cfg.BaseURL)
		...
		var result models.ImportTableOutput
...
func CreateImporttableTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_ImportTable",
		mcp.WithDescription(" Imports table data from an S3 bucket. "),
		...
		mcp.WithString("InputFormat", mcp.Required(), mcp.Description("Input parameter:  The format of the source data. Valid values for <code>ImportFormat</code> are <code>CSV</code>, <code>DYNAMODB_JSON</code> or <code>ION</code>. ")),
		mcp.WithString("InputFormatOptions", mcp.Description("Input parameter:  Additional properties that specify how the input is formatted, ")),
		mcp.WithString("S3BucketSource", mcp.Required(), mcp.Description("Input parameter:  The S3 bucket that provides the source for the import. ")),
		mcp.WithString("TableCreationParameters", mcp.Required(), mcp.Description("Input parameter: Parameters for the table to import the data into. ")),
		...
		mcp.WithString("InputCompressionType", mcp.Description("Input parameter:  Type of compression to be used on the input coming from the imported table. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ImporttableHandler(cfg),
	}
}
