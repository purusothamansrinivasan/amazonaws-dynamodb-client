...
func ListcontributorinsightsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ListContributorInsightsInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.ListContributorInsights%s", cfg.BaseURL, queryString)
		...
		var result models.ListContributorInsightsOutput
...
func CreateListcontributorinsightsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_ListContributorInsights",
		mcp.WithDescription("Returns a list of ContributorInsightsSummary for a table and all its global secondary indexes."),
		...
		mcp.WithString("NextToken", mcp.Description("Input parameter: A token to for the desired page, if there is one.")),
		mcp.WithString("TableName", mcp.Description("Input parameter: The name of the table.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListcontributorinsightsHandler(cfg),
	}
}
