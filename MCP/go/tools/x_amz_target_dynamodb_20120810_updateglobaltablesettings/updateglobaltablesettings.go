...
func UpdateglobaltablesettingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.UpdateGlobalTableSettingsInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.UpdateGlobalTableSettings", cfg.BaseURL)
		...
		var result models.UpdateGlobalTableSettingsOutput
...
func CreateUpdateglobaltablesettingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_UpdateGlobalTableSettings",
		...
		mcp.WithString("GlobalTableName", mcp.Required(), mcp.Description("Input parameter: The name of the global table")),
		mcp.WithString("GlobalTableProvisionedWriteCapacityAutoScalingSettingsUpdate", mcp.Description("Input parameter: Auto scaling settings for managing provisioned write capacity for the global table.")),
		mcp.WithString("GlobalTableProvisionedWriteCapacityUnits", mcp.Description("Input parameter: The maximum number of writes consumed per second before DynamoDB returns a <code>ThrottlingException.</code> ")),
		mcp.WithString("ReplicaSettingsUpdate", mcp.Description("Input parameter: Represents the settings for a global table in a Region that will be modified.")),
		mcp.WithString("GlobalTableBillingMode", mcp.Description("Input parameter: <p>The billing mode of the global table. If <code>GlobalTableBillingMode</code> is not specified, the global table defaults to <code>PROVISIONED</code> capacity billing mode.</p> <ul> <li> <p> <code>PROVISIONED</code> - We recommend using <code>PROVISIONED</code> for predictable workloads. <code>PROVISIONED</code> sets the billing mode to <a href=\"https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.ReadWriteCapacityMode.html#HowItWorks.ProvisionedThroughput.Manual\">Provisioned Mode</a>.</p> </li> <li> <p> <code>PAY_PER_REQUEST</code> - We recommend using <code>PAY_PER_REQUEST</code> for unpredictable workloads. <code>PAY_PER_REQUEST</code> sets the billing mode to <a href=\"https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.ReadWriteCapacityMode.html#HowItWorks.OnDemand\">On-Demand Mode</a>. </p> </li> </ul>")),
		mcp.WithString("GlobalTableGlobalSecondaryIndexSettingsUpdate", mcp.Description("Input parameter: Represents the settings of a global secondary index for a global table that will be modified.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdateglobaltablesettingsHandler(cfg),
	}
}
