...
func DisablekinesisstreamingdestinationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.DisableKinesisStreamingDestination", cfg.BaseURL)
...
func CreateDisablekinesisstreamingdestinationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_DisableKinesisStreamingDestination",
		mcp.WithDescription("Stops replication from the DynamoDB table to the Kinesis data stream. This is done without deleting either of the resources."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DisablekinesisstreamingdestinationHandler(cfg),
	}
}
