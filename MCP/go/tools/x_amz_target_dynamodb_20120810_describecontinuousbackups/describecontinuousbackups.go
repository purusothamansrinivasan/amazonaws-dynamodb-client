...
func DescribecontinuousbackupsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DescribeContinuousBackupsInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.DescribeContinuousBackups", cfg.BaseURL)
		...
		var result models.DescribeContinuousBackupsOutput
...
func CreateDescribecontinuousbackupsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_DescribeContinuousBackups",
		...
		mcp.WithString("TableName", mcp.Required(), mcp.Description("Input parameter: Name of the table for which the customer wants to check the continuous backups and point in time recovery settings.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DescribecontinuousbackupsHandler(cfg),
	}
}
