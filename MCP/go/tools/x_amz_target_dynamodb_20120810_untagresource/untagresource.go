...
func UntagresourceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.UntagResource", cfg.BaseURL)
...
func CreateUntagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_UntagResource",
		...
		mcp.WithString("ResourceArn", mcp.Required(), mcp.Description("Input parameter: The DynamoDB resource that the tags will be removed from. This value is an Amazon Resource Name (ARN).")),
		mcp.WithString("TagKeys", mcp.Required(), mcp.Description("Input parameter: A list of tag keys. Existing tags of the resource whose keys are members of this list will be removed from the DynamoDB resource.")),
	)
