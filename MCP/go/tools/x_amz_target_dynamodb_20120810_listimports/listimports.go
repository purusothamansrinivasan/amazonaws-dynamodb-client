...
func ListimportsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ListImportsInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.ListImports%s", cfg.BaseURL, queryString)
		...
		var result models.ListImportsOutput
...
func CreateListimportsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_ListImports",
		mcp.WithDescription(" Lists completed imports within the past 90 days. "),
		...
		mcp.WithString("PageSize", mcp.Description("Pagination limit")),
		mcp.WithString("NextToken", mcp.Description("Pagination token")),
		mcp.WithString("X-Amz-Target", mcp.Required(), mcp.Description("")),
		mcp.WithString("PageSize", mcp.Description("Input parameter:  The number of <code>ImportSummary </code>objects returned in a single page. ")),
		mcp.WithString("TableArn", mcp.Description("Input parameter:  The Amazon Resource Name (ARN) associated with the table that was imported to. ")),
		mcp.WithString("NextToken", mcp.Description("Input parameter:  An optional string that, if supplied, must be copied from the output of a previous call to <code>ListImports</code>. When provided in this manner, the API fetches the next page of results. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListimportsHandler(cfg),
	}
}
