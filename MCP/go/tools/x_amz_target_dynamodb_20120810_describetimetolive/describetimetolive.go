...
func DescribetimetoliveHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DescribeTimeToLiveInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.DescribeTimeToLive", cfg.BaseURL)
		...
		var result models.DescribeTimeToLiveOutput
...
func CreateDescribetimetoliveTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_DescribeTimeToLive",
		mcp.WithDescription("Gives a description of the Time to Live (TTL) status on the specified table. "),
		...
		mcp.WithString("TableName", mcp.Required(), mcp.Description("Input parameter: The name of the table to be described.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DescribetimetoliveHandler(cfg),
	}
}
