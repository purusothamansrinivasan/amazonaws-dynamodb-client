...
func DescribeexportHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DescribeExportInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.DescribeExport", cfg.BaseURL)
		...
		var result models.DescribeExportOutput
...
func CreateDescribeexportTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_DescribeExport",
		mcp.WithDescription("Describes an existing table export."),
		...
		mcp.WithString("ExportArn", mcp.Required(), mcp.Description("Input parameter: The Amazon Resource Name (ARN) associated with the export.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DescribeexportHandler(cfg),
	}
}
