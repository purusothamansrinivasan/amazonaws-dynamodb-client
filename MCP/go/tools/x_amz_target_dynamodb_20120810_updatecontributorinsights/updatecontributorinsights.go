...
func UpdatecontributorinsightsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.UpdateContributorInsightsInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.UpdateContributorInsights", cfg.BaseURL)
		...
		var result models.UpdateContributorInsightsOutput
...
func CreateUpdatecontributorinsightsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_UpdateContributorInsights",
		mcp.WithDescription("Updates the status for contributor insights for a specific table or index. CloudWatch Contributor Insights for DynamoDB graphs display the partition key and (if applicable) sort key of frequently accessed items and frequently throttled items in plaintext. If you require the use of Amazon Web Services Key Management Service (KMS) to encrypt this table’s partition key and sort key data with an Amazon Web Services managed key or customer managed key, you should not enable CloudWatch Contributor Insights for DynamoDB for this table."),
		...
		mcp.WithString("ContributorInsightsAction", mcp.Required(), mcp.Description("Input parameter: Represents the contributor insights action.")),
		mcp.WithString("IndexName", mcp.Description("Input parameter: The global secondary index name, if applicable.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdatecontributorinsightsHandler(cfg),
	}
}
