...
func ExecutetransactionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ExecuteTransactionInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.ExecuteTransaction", cfg.BaseURL)
		...
		var result models.ExecuteTransactionOutput
...
func CreateExecutetransactionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_ExecuteTransaction",
		...
		mcp.WithString("ClientRequestToken", mcp.Description("Input parameter: Set this value to get remaining results, if <code>NextToken</code> was returned in the statement response.")),
		...
		mcp.WithString("TransactStatements", mcp.Required(), mcp.Description("Input parameter: The list of PartiQL statements representing the transaction to run.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ExecutetransactionHandler(cfg),
	}
}
