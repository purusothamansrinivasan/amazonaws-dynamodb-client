...
func UpdatecontinuousbackupsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.UpdateContinuousBackupsInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.UpdateContinuousBackups", cfg.BaseURL)
		...
		var result models.UpdateContinuousBackupsOutput
...
func CreateUpdatecontinuousbackupsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_UpdateContinuousBackups",
		mcp.WithDescription("<p> <code>UpdateContinuousBackups</code> enables or disables point in time recovery for the specified table. A successful <code>UpdateContinuousBackups</code> call returns the current <code>ContinuousBackupsDescription</code>. Continuous backups are <code>ENABLED</code> on all tables at table creation. If point in time recovery is enabled, <code>PointInTimeRecoveryStatus</code> will be set to ENABLED.</p> <p> Once continuous backups and point in time recovery are enabled, you can restore to any point in time within <code>EarliestRestorableDateTime</code> and <code>LatestRestorableDateTime</code>. </p> <p> <code>LatestRestorableDateTime</code> is typically 5 minutes before the current time. You can restore your table to any point in time during the last 35 days. </p>"),
		...
		mcp.WithString("PointInTimeRecoverySpecification", mcp.Required(), mcp.Description("Input parameter: Represents the settings used to enable point in time recovery.")),
		mcp.WithString("TableName", mcp.Required(), mcp.Description("Input parameter: The name of the table.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdatecontinuousbackupsHandler(cfg),
	}
}
