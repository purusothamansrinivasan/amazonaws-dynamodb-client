...
func DescribekinesisstreamingdestinationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DescribeKinesisStreamingDestinationInput
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=DynamoDB_20120810.DescribeKinesisStreamingDestination", cfg.BaseURL)
		...
		var result models.DescribeKinesisStreamingDestinationOutput
...
func CreateDescribekinesisstreamingdestinationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=DynamoDB_20120810_DescribeKinesisStreamingDestination",
		mcp.WithDescription("Returns information about the status of Kinesis streaming."),
		...
		mcp.WithString("TableName", mcp.Required(), mcp.Description("Input parameter: The name of the table being described.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DescribekinesisstreamingdestinationHandler(cfg),
	}
}
